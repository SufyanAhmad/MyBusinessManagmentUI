...
import { AddedBusinessUnitModel, BusinessUnitModel, UserModel } from '../../../models/super-admin/super-admin-model';
...
@Component({
  selector: 'app-users',
  imports: [MatPaginatorModule, MatSortModule, MatTableModule, RouterLink, FormsModule, ReactiveFormsModule, ButtonModule, DialogModule, CommonModule, CheckboxModule, SelectModule, SkeletonModule, ToastModule, LoadingComponent, DataNotFoundComponent, ConfirmationPopupComponent],
...
RestUserPasswordForm!: FormGroup;
 displayedColumns: string[] = ['img','name', 'email', 'actions','forget'];
  ...
  ngOnInit() {
     this.RestUserPasswordForm = this.formBuilder.group({
              ...
              newPassword: ['', [Validators.required, Validators.minLength(8), Validators.pattern('(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%*?&])[A-Za-zd$@$!%*?&].{7,}')]],
         })
    this.initForm();
    ...
    merge(this.paginator.page)
      .pipe(
        ...
        switchMap(() => {
          ...
          return this.superAdminService.getUserBySearchFilter
            (data)
            .pipe(
              catchError((resp: any) => {
                if (resp.status == 401) {
 ...
 this.addUserModel = this.formBuilder.group({
              ...
              password: ['', [Validators.required, Validators.minLength(8), Validators.pattern('(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%*?&])[A-Za-zd$@$!%*?&].{7,}')]],
         })
  }
    ...
    this.superAdminService.addUser(this.addUserModel.value).subscribe((dt) => {
      this.addLoading = false;
      this.visible=false;
      this.getUsers();
      this.messageService.add({ severity: 'success', summary: 'Added', detail: 'User added successfully', life: 3000 });
  ...
  this.accountService.ResetPassword(this.RestUserPasswordForm.value).subscribe(
        (dt) => {
          this.messageService.add({ severity: 'success', summary: 'Reset', detail: 'Password reset successfully.', life: 3000 });
...
checkedBusinessUnitCounts(item:any) {
  ...
  this.userBusinessUnitAndRoleDtosCount=0;
    this.userBusinessUnitAndRoleDtosCount=this.getSelectedBusinessUnits().length;
...
submitAssignedUnits() {
  ...
  this.superAdminService.assignBusinessUnitToUser(data).subscribe(
    (response) => {
      ...
      this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Business units assigned successfully', life: 3000 });
      // this.getUsers();
    },
    (error) => {
      this.assignLoading = false;
...
isValidAssignment(): boolean {
  ...
  const allHaveRoles = allSelectedItems.every(item => item.userRoleId != null&&item.userRoleId !== undefined);
  return allHaveRoles;
}
// 
checkedEditBusinessUnitCounts(item:any) {
  ...
  this.userEditBusinessUnitAndRoleDtosCount=0;
    this.userEditBusinessUnitAndRoleDtosCount=this.getEditSelectedBusinessUnits().length;
...
submitAddedAssignedUnits() {
  ...
  this.superAdminService.updateBusinessUnitToUser(this.applicationUserId,userBusinessUnitAndRoleDtos).subscribe(
    (response) => {
      this.editLoading = false;
      this.isUpdatePopup = false;
      this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Business units updated successfully', life: 3000 });
  ...
  editUserStatus(userId:any,status:any){
    ...
    this.superAdminService.changeUserStatus(data).subscribe((dt) => {
      this.messageService.add({ severity: 'success', summary: 'Updated', detail: 'User status update successfully', life: 3000 });
      // this.getSuppliers();
    },
      (error) => {
        this.addLoading = false;
...
deleteUser(){
    this.showConfirmDialog=false;
     this.superAdminService.deleteUser(this.userId).subscribe((dt) => {
       this.messageService.add({ severity: 'success', summary: 'Deleted', detail: 'User deleted successfully', life: 3000 });
     },
       (error) => {     
         this.messageService.add({ severity: 'error', summary: 'Error', detail: error.error?.message, life:3000 });
         if (error.status == 401) {
