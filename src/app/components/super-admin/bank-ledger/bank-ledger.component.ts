...
export class BankLedgerComponent {
  ...
  bankConfirmDialog: boolean = false;
  resultsLength: any = 0;
  ...
  openDeleteConfirmationDialog(){
    this.bankConfirmDialog = true;
  }
    handleBankCancelled() {
    this.bankConfirmDialog = false;
  }
  // delete cold store
  deleteBank() {
    this.bankConfirmDialog = false;
    // this.superAdminService.DeleteBank(this.deletedItemId).subscribe(
    //   (data) => {
    //     this.getBankLedgerList();
    //     this.messageService.add({
    //       severity: 'success',
